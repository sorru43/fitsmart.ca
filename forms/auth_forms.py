...
from wtforms import StringField, PasswordField, BooleanField, SubmitField, SelectField, TextAreaField, TimeField, IntegerField
...
class WaterReminderForm(FlaskForm):
    """Form for water reminder preferences"""
    is_active = BooleanField('Enable Water Reminders')
    start_time = TimeField('Start Time', validators=[DataRequired()])
    end_time = TimeField('End Time', validators=[DataRequired()])
    interval_minutes = IntegerField('Reminder Interval (minutes)', validators=[DataRequired()], default=60)
    daily_target_ml = IntegerField('Daily Target (ml)', validators=[DataRequired()], default=2000)
    wake_up_time = TimeField('Wake Up Time')
    sleep_time = TimeField('Sleep Time')
