...
# Try to import fpdf2 first, fallback to fpdf
try:
    ...
    try:
        ...
        class FPDF:
            def __init__(self, *args, **kwargs):
                raise ImportError("Neither fpdf2 nor fpdf is installed. Please install one of them.")
...
def get_daily_orders(date=None):
    """
    Get orders for a specific date. For testing, returns sample orders.
    ...
    Returns:
        list: List of active order dictionaries
    """
    # For testing, return sample orders
    sample_orders = create_sample_orders(date)
    
    # Filter out skipped meals and inactive subscriptions
    active_orders = [
        order for order in sample_orders 
        if not order.get('is_skipped', False) 
        and order.get('subscription_status', '') == 'ACTIVE'
    ]
...
def email_daily_orders_report(date=None, recipient_email=None):
    ...
    location_counts = {}
    for order in orders:
        location_key = f"{order['delivery_city']}, {order['delivery_province']}"
