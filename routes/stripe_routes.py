...
import json
import logging
from flask import Blueprint, request, jsonify, redirect, url_for, flash, render_template, session, current_app
...
@stripe_bp.route('/stripe-create-checkout-session/<int:plan_id>', methods=['POST'])
def stripe_create_checkout_session(plan_id):
    ...
    try:
        ...
        success_url = url_for('stripe.stripe_checkout_success', _external=True)
        cancel_url = url_for('stripe.stripe_checkout_cancel', _external=True)
...
@stripe_bp.route('/customer-portal')
def customer_portal():
    """
...
@csrf.exempt  # Exempt from CSRF protection since Stripe calls this directly
def stripe_webhook():
    """
    Handle Stripe webhook events.
    Note: This route is exempt from CSRF protection since it's called by Stripe.
    """
